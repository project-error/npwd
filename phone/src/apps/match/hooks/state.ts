...
export const matchState = {
  profiles: atom<FormattedProfile[]>({
    ...
    default: selector({
      ...
      get: async () => {
        try {
          const resp = await fetchNui<FormattedProfile[]>(MatchEvents.GET_PROFILES);
          LogDebugEvent({ action: 'fetchProfiles', data: resp });
          return resp ?? [];
        } catch (e) {
          if (isEnvBrowser()) {
  ...
  matches: atom<FormattedMatch[]>({
    ...
    default: selector({
      ...
      get: async () => {
        try {
          const resp = await fetchNui<FormattedMatch[]>(MatchEvents.GET_MATCHES);
          LogDebugEvent({ action: 'fetchMatches', data: resp });
          return resp ?? [];
        } catch (e) {
          if (isEnvBrowser()) {
  ...
  myProfile: atom<FormattedProfile | undefined>({
    key: 'myProfile',
    default: selector({
      key: 'myProfileDefault',
      get: async () => {
        try {
          const resp = await fetchNui<FormattedProfile>(MatchEvents.GET_MY_PROFILE);
          LogDebugEvent({ action: 'fetchMyProfile', data: resp });
          return resp;
        } catch (e) {
          if (isEnvBrowser()) {
