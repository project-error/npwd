...
jest.mock('../../../../os/phone/hooks/usePhone', () => ({ 
    usePhone: jest.fn().mockReturnValue({ config: {
        twitter: {
...
jest.mock('../../hooks/useTwitterNotification', () => ({ 
    useTwitterNotification: jest.fn().mockReturnValue({ notification:  {
        profile_name: 'test',
...
describe('TwitterNotification', () => {
    ...
    it('renders message', () => {
        expect(wrapper.find('.makeStyles-message-7').text()).toEqual('test message');
    });
});
