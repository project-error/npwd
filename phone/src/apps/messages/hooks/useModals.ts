...
export default (): IUseModal => {
  ...
  ] = useRecoilState<MessageGroup | null>(messageState.activeMessageGroup);
  const [showNewMessageGroup, setShowNewMessageGroup] = useRecoilState<boolean>(
    messageState.showNewMessageGroup
  );
  return {
    activeMessageGroup,
    setActiveMessageGroup,
    showNewMessageGroup,
    setShowNewMessageGroup,
  };
};
