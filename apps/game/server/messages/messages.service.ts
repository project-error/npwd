...
class _MessagesService {
	...
	async handleCreateMessageConversation(
		...
		const playerPhoneNumber = PlayerService.getPlayer(
			reqObj.source,
		).getPhoneNumber();
		...
		const doesExist =
			await this.messagesDB.doesConversationExist(conversationList);

		if (doesExist) {
			const playerHasConversation =
				await this.messagesDB.doesConversationExistForPlayer(
					conversationList,
					playerPhoneNumber,
				);
			...
			if (playerHasConversation) {
				...
				return resp({
					status: "ok",
					data: { ...respData, participant: playerPhoneNumber },
				});
			}
		}
			...
			for (const participant of participants) {
				if (participant !== playerPhoneNumber) {
					const participantIdentifier =
						await PlayerService.getIdentifierByPhoneNumber(participant);
					const participantPlayer = PlayerService.getPlayerFromIdentifier(
	...
	async handleSendMessage(
		...
		try {
			...
			const participants = getIdentifiersFromParticipants(
			...
			for (const participantId of participants) {
				if (participantId !== authorPhoneNumber) {
					try {
						...
						const participantIdentifier =
							await PlayerService.getIdentifierByPhoneNumber(
								participantId,
								true,
							);

						const participantNumber =
							await PlayerService.getPhoneNumberFromIdentifier(
								participantIdentifier,
							);
						...
						messagesLogger.warn(
							`Failed to broadcast message. Player is not online.`,
						);
					}
				}
			}
	...
	async handleEmitMessage(dto: EmitMessageExportCtx) {
		...
		try {
			...
			const senderPlayer = await PlayerService.getIdentifierFromPhoneNumber(
				senderNumber,
				true,
			);

			const participantIdentifier =
				await PlayerService.getIdentifierFromPhoneNumber(targetNumber);
			const participantPlayer = PlayerService.getPlayerFromIdentifier(
				participantIdentifier,
			);
			...
			if (!doesConversationExist) {
				...
				conversationId =
					await this.messagesDB.getConversationId(conversationList);
			}
	...
	async handleGetLocation(
		...
		resp: PromiseEventResp<Location>,
	) {
		const phoneNumber = PlayerService.getPlayer(reqObj.source).getPhoneNumber();
