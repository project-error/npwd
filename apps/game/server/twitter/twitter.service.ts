...
class _TwitterService {
	...
	async handleCreateProfile(
		...
		try {
			const identifier = PlayerService.getIdentifier(reqObj.source);
			const profile = await this.twitterDB.createProfile(
				identifier,
				reqObj.data.profile_name,
			);
	...
	async handleUpdateProfile(
		...
		try {
			...
			const profile = await this.twitterDB.updateProfile(
				identifier,
				reqObj.data,
			);
			...
			resp({
				status: "error",
				errorMsg: "TWITTER.FEEDBACK.EDIT_PROFILE_FAILURE",
			});
		}
	}
	...
	async handleFetchTweetsFiltered(
		...
		try {
			const identifier = PlayerService.getIdentifier(reqObj.source);
			const profile = await this.twitterDB.getProfile(identifier);
			const tweets = await this.twitterDB.fetchTweetsFiltered(
				profile.id,
				reqObj.data.searchValue,
			);
			...
			resp({
				status: "error",
				errorMsg: "TWITTER.FEEDBACK.FILTERED_FETCH_FAILED",
			});
		}
	}
	...
	async handleCreateTweet(
		...
		try {
			...
			const createdTweet = await this.twitterDB.createTweet(
				identifier,
				reqObj.data,
			);
	...
	async handleToggleLike(
		...
		try {
			const identifier = PlayerService.getIdentifier(reqObj.source);
			const profile = await this.twitterDB.getOrCreateProfile(identifier);
			const likeExists = await this.twitterDB.doesLikeExist(
				profile.id,
				reqObj.data.tweetId,
			);
	...
	async handleRetweet(
		...
		try {
			...
			if (
				await this.twitterDB.doesRetweetExist(reqObj.data.tweetId, identifier)
			) {
				return resp({
					status: "error",
			...
			const retweet: NewTweet = {
				message: "",
				images: "",
				retweet: reqObj.data.tweetId,
			};
			const createdTweet = await this.twitterDB.createTweet(
				identifier,
				retweet,
	...
	async handleReport(
		...
		try {
			const identifier = PlayerService.getIdentifier(reqObj.source);
			const profile = await this.twitterDB.getProfile(identifier);
			const tweet = await this.twitterDB.getTweet(
				profile.id,
				reqObj.data.tweetId,
			);

			const reportExists = await this.twitterDB.doesReportExist(
				tweet.id,
				profile.id,
			);
			...
			if (reportExists) {
				return twitterLogger.warn(
					"This profile has already reported this tweet",
				);
			}
