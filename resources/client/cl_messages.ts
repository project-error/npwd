 ...
 */
RegisterNuiCallbackType(events.MESSAGES_GET_MESSAGE_GROUPS);
on(`__cfx_nui:${events.MESSAGES_GET_MESSAGE_GROUPS}`, (): void => {
  emitNet(events.MESSAGES_GET_MESSAGE_GROUPS);
});

onNet(events.MESSAGES_GET_MESSAGE_GROUPS_SUCCESS, (messageGroups: MessageGroup[]): void => {
  sendMessageEvent(events.MESSAGES_GET_MESSAGE_GROUPS_SUCCESS, messageGroups);
  emitNet(events.MESSAGES_GET_MESSAGE_GROUPS_SUCCESS);
});

onNet(events.MESSAGES_CREATE_MESSAGE_GROUP_FAILED, () => {
...
onNet(events.MESSAGES_CREATE_MESSAGE_GROUP_SUCCESS, (result: any): void => {
  ...
  emitNet(events.MESSAGES_CREATE_MESSAGE_GROUP_SUCCESS);
});
